 ...
 * @author 84348
 */
@WebServlet(name = "SearchIssueDateController", urlPatterns = {"/SearchIssueDateController"})
public class SearchIssueDateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(session!=null){
                request.removeAttribute("LIST_ISSUE");
            }
            String search = request.getParameter("search");
            String seacrh1 = request.getParameter("searchR");
            DAOIssue dao = new DAOIssue();
            List<UserIssueS> listIssue = dao.getListSearchIssueDate(search, seacrh1);
            if(listIssue.size() >0){
